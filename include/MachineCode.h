...
{
public:
    enum opType { ADD, SUB, MUL, DIV, AND, OR, MOD, XOR };
...
public:
    ...
    void InsertFron(MachineInstruction* inst) { this->inst_list.insert(this->inst_list.begin(),inst);};
...
private:
    ...
    int stack_size;
    std::vector<MachineInstruction*> retpatch;
    SymbolEntry* sym_ptr;
public:
    ...
    * you can use this function in LinearScan::genSpillCode() */
    ...
    void SSSize(int size){stack_size += size;};
